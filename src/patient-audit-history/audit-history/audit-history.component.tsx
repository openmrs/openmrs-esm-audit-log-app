...
const AuditHistory: React.FC<AuditHistoryProps> = ({ patientUuid, patient }) => {
  ...
  const { viewPrivilege, auditHistoryPageSize } = useConfig<ConfigObject>().patientAuditHistory;
    ...
    return (
      <Layer>
        <Tile className={styles.messageTile}>
          <p>{t('auditHistoryNoPermission', "You do not have permission to view this patient's audit history.")}</p>
        </Tile>
      </Layer>
    );
  }
      ...
      return (
        <Layer>
          <Tile className={styles.messageTile}>
            <p>
              {t(
                'auditHistoryModuleNotInstalled',
